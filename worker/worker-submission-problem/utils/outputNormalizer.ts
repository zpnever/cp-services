...
export const normalizeOutput = (
	...
	// Normalize booleans
	if (
		expectedOutput.toLowerCase() === "true" ||
...
const normalizeFloatOutput = (
	...
	const expectedPrecision =
		expectedDecimalParts.length > 1 ? expectedDecimalParts[1].length : 0;
...
export const compareOutputs = (
	...
	const normalizedOutput = normalizeOutput(output, expectedOutput, languageId);
	...
	if (expectedOutput.startsWith("[") && expectedOutput.endsWith("]")) {
		try {
			...
			if (Array.isArray(expectedArray) && Array.isArray(outputArray)) {
				if (expectedArray.length !== outputArray.length) return false;

				for (let i = 0; i < expectedArray.length; i++) {
					// Handle primitive values
					if (expectedArray[i] !== outputArray[i]) return false;
				}
				return true;
			}
		} catch (e) {
